...
public class BaseClient {
    ...
    protected String addParamToPath(String path, Map<String, Object> parameters) {
        StringBuilder pathWithParams = new StringBuilder(path);
        pathWithParams.append("?");

        for (String paramName : parameters.keySet()) {
            pathWithParams.append(paramName)
                    .append("={")
                    .append(paramName)
                    .append("}&");
        }
        String test = pathWithParams.substring(0, pathWithParams.length() - 1);
